...
public class RunMe {
    public static <OptionalInt> void main(String[] args) {
        ...
        Map<String, List<Employee>> empByCond;
        empByCond = empList.stream().filter(s->s.getSalary()>120).collect(Collectors.groupingBy(Employee::getDeptName));
        System.out.println("Employees grouped by dep with sal more than 120 :: \n" + empByCond);

        //2 Group the Employees by city.
        ...
        //4 Find the count of male and female employees present in the organization.

        Map<String, Long> noOfMaleAndFemaleEmployees  = empList.stream().collect(Collectors.
        ...
        //5 Print the names of all departments in the organization.
       ...
       //6 Print employee details whose age is greater than 28.
         empList.stream().filter(s->s.getAge()>=32).toList().forEach(System.out::println);


       // 7 Find maximum age of employee.


        java.util.OptionalInt max =  empList.stream().mapToInt(Employee::getAge).max();
        System.out.println("Maximum age of Employee: " + max.getAsInt());

//8Print Average age of Male and Female Employees.
       ...
       //9 Print the number of employees in each department.
...
//can use anyMatch also
        Optional<Employee> youngestEmp = empList.stream().filter(e -> e.getGender().equalsIgnoreCase("F"))
      ...
      //  11. Find employees whose age is greater than 30 and less than 30.
       ...
       // 25. Find Second Highest salary in the organisation.
        ...
        emp2.ifPresent(employee -> System.out.println("Second Highest Salary in the organisation : " + employee.getSalary()));
        //26. Nth Highest salary.

        int n = 10;// this can be any nth number highest salary
        Optional<Employee> empVal = empList.stream().sorted(Comparator.comparingDouble(Employee::getSalary)
                .reversed()).skip(n-1).findFirst();
        empVal.ifPresent(e->System.out.println("Second Highest Salary in the organisation : " + e.getSalary()));
